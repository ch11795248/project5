...
public class Myrealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //设置权限
        List<Permission> perms = rolePermsService.getPermsByAccount(account);
        Set<String> permString = new HashSet<>();
        if(null != perms && perms.size()>0 && null !=perms.get(0)) {
            for (Permission permission : perms) {
                if(null != permission && !"".equals(permission.getPermission()) && null !=permission.getPermission()){
