                            ...
                            <table class="table" border="1">
                                ...
                                <c:forEach items="${pageInfo.list}" var="role">
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-warning" onclick="showselectmodel(${role.id})">修改
                                            </button>
                                            <button class="btn btn-danger" onclick="deleterRoleById(${role.id})">删除
                                            </button>
                                        </td>
                                    </tr>
                                </c:forEach>
                    ...
                    <c:forEach items="${allPerms}" var="perm">
                        <tr>
                            <td><input type="checkbox" name="checkedemployee"
                                       class="checkedbox1" value="${perm.id}" style="opacity: 1;"/></td>
                            <td>${perm.id}</td>
                            <td>${perm.permName}</td>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${perm.status eq 1}">正常</c:when>
...
<div id="myModal2" class="modal">
    <div class="modal-dialog" style="left: 0;">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-info" data-dismiss="modal" onclick="updateRoleInfo()" id="btn">确认</button>
                <button type="button" class="btn btn-warning" data-dismiss="modal">取消</button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function showselectmodel(id) {
        ...
        $('#example').DataTable({
            "aLengthMenu": [[8, 10, 25, -1], [8, 10, 25, "所有"]],//设置每页显示数据条数的下拉选项
